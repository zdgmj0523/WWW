    ...
    $.ajax({
        ...
        success:function (response,xhr) {
            ...
            switch(response.payStatus) {
                case '2':
                    $('#payStatus').html("提现状态：2-代付成功");
                    break;
                case '3':
                    $('#payStatus').html("提现状态：3-代付异常");
                    break;
                default:
                    $('#payStatus').html("提现状态：4-代付失败");
            }
            switch(response.transStatus){
                case '2':
                    $('#transStatus').html("代付状态：2-提现成功");
                    break;
                case '3':
                    $('#transStatus').html("代付状态：3-提现异常");
                    break;
                default:
                    $('#transStatus').html("代付状态：4-提现失败");
            }
        }
    });
}
